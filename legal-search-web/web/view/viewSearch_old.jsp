...
//        String IP = "127.0.0.1";
	...
	queryJ.put("n", 20);                
        ...
        String newString = new String(queryJ.toString().getBytes("UTF-8"), "UTF-8");
        
        JSONObject o = new JSONObject( remote.query2(newString));
...
<form method="get">
    ...
    Query: <input class='search-box' type="text" name="q" value="<%=_vQuery%>" size="60"/>
     ...
     <p>
     Query space:
     <div style="display: block; width: 400px; margin-left: 40px"><input name='space'  type="radio" value="org" <% if(space.equals("org")) out.print("checked='checked'"); %>/> Original space (lemur API) </div>
     <div style="display: block; width: 400px; margin-left: 40px"><input name='space' type="radio" value="mds" <% if(space.equals("mds")) out.print("checked='checked'"); %>/> MDS space (python API)</div>
